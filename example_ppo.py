...
tracker = unstable.Tracker.options(name="Tracker").remote(
    run_name=f"ppo-test-{MODEL_NAME.split('/')[-1]}-{env_sampler.env_list()}-{int(time.time())}", 
...
learner = unstable.PPOLearner.options(num_gpus=1, name="Learner").remote(
...
ray.get(learner.initialize_algorithm.remote(infer_mini_batch_size=4, update_epochs=2, critic_learning_rate=5e-5, normalize_adv=True, max_train_len=MAX_TRAIN_SEQ_LEN, max_generation_len=MAX_GENERATION_LENGTH))
