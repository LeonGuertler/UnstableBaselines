...
def masked_mean(x, mask, axis=None): 
    return (x * mask).sum(dim=axis) / mask.sum(dim=axis) if axis else (x * mask).sum() / mask.sum()

def masked_std(x, mask, axis=None, eps: float = 1e-8):
    if axis is None: return x[mask.bool()].std(unbiased=False).clamp_min(eps)
    mean = masked_mean(x, mask, axis=axis).unsqueeze(axis)
