...
class A2CLearner(BaseLearner):
    ...
    def _mini_batch_update_step(self, input_ids, attention_mask, response_mask, returns, values, advantages) -> Dict[str, float]:
        # Policy Update
        logps, entropy, _ = self._forward_minibatch(input_ids.to(self.policy_device), attention_mask.to(self.policy_device), response_mask.to(self.policy_device), self.policy_model)
        policy_loss = -logps * advantages.detach()
        ...
        value_loss = (value_pred - returns).pow(2)
        ...
        return {
            ...
            "logp_mean": masked_mean(logps, response_mask.to(self.policy_device)).item(),
            "logp_std": masked_std(logps, response_mask.to(self.policy_device)).item(),
    ...
    def _update(self, batch):
        ...
        for _ in range(self.update_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                update_metrics = self._mini_batch_update_step(mb_input_ids, mb_attention_mask, mb_response_mask, mb_returns, mb_values, mb_advantages)
                if (i+1) % self.actor_grad_accumulation_steps == 0:
                    torch.nn.utils.clip_grad_norm_(self.policy_model.parameters(), self.grad_clip)
                    grad_norm = self.policy_optimizer.step()
                ...
                if (i+1) % self.critic_grad_accumulation_steps == 0:
                    ...
                    grad_norm = self.critic_optimizer.step()
