    ...
    def train(self, iterations: int):
        ...
        while self._step < iterations:
            try:
                ...
                log.update({"step": self._step,  "samples_seen": self._samples_seen,  "lr": self.policy_optimizer.param_groups[0]["lr"]})
                if "policy_grad_norm" not in log: log.update({"policy_grad_norm": sum(p.grad.data.norm(2).item()**2 for p in self.policy_model.parameters() if p.grad is not None) ** 0.5})
