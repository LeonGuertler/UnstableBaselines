...
def _load_lora_state(model, lora_path: str, adapter_name: Optional[str] = None, revision: Optional[str] = None):
    resolved_path = _resolve_checkpoint_path(lora_path, revision)
    adapter_name = adapter_name or getattr(model, "actor_adapter_name", "default")
    model.load_adapter(resolved_path, adapter_name=adapter_name, is_trainable=True)
    print(f"[_load_lora_state] ✅ Loaded LoRA adapter '{adapter_name}' from {resolved_path}")

def _load_value_head(model, ckpt_path: str, value_head_prefix: str = "value_head", revision: Optional[str] = None):
    resolved_path = _resolve_checkpoint_path(ckpt_path, revision)
    vh_path = os.path.join(resolved_path, f"{value_head_prefix}.pt")
    if os.path.exists(vh_path):
        vh = getattr(model.base_model, value_head_prefix)
        vh.load_state_dict(torch.load(vh_path, weights_only=True))
        print(f"[_load_value_head] ✅ Loaded value head from {vh_path}")
    else: print(f"[_load_value_head] ⚠️  No value head file found at {vh_path}, using initialized weights")
...
def build_peft_model(base_name: str, device: torch.device, lora_cfg: Dict[str, Any]|None, checkpoint_cfg: Dict[str, Any]|None, freeze_base: bool=True, value_head: bool=False, value_head_prefix: str="value_head") -> Tuple[torch.nn.Module, "transformers.PreTrainedTokenizer"]:
    ...
    model.adapter_name = getattr(model, "active_adapter", "default")
    tok = AutoTokenizer.from_pretrained(base_name, trust_remote_code=True, use_fast=True)
    tok.pad_token = tok.eos_token
    if checkpoint_cfg.get('path'): 
        _load_lora_state(model, checkpoint_cfg['path'], adapter_name=model.adapter_name, revision=checkpoint_cfg.get('revision'))
        if value_head: _load_value_head(model, checkpoint_cfg['path'], value_head_prefix=value_head_prefix, revision=checkpoint_cfg.get('revision'))
    return model, tok
