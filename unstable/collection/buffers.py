...
class StepBuffer(BaseBuffer):
    ...
    def add_player_trajectory(self, player_traj: PlayerTrajectory, env_id: str):
        ...
        for idx in indices:
            ...
            action = player_traj.actions[idx] if idx < len(player_traj.actions) else None
            with self.mutex: self.steps.append(Step(pid=player_traj.pid, obs=player_traj.obs[idx], prompt=player_traj.prompts[idx], prompt_ids=player_traj.prompt_ids[idx], completion=player_traj.completions[idx], completion_ids=player_traj.completion_ids[idx], completion_logprobs=player_traj.completion_logprobs[idx], reward=step_reward, env_id=env_id, step_info={"raw_reward": player_traj.final_reward, "env_reward": reward, "step_reward": step_reward, "action": action}))
...
class EpisodeBuffer(BaseBuffer):
    def __init__(
        ...
        self._pending_groups: dict = {}  # group_id -> list of episodes staging until full
        self.mutex = Lock()

    def _group(self) -> List:
        groups: dict = {}
        for ep in self.episodes:
            key = ep[0].group_id if ep and ep[0].group_id is not None else id(ep)
    ...
    def add_player_trajectory(self, player_traj: PlayerTrajectory, env_id: str):
        ...
        for idx in list(range(len(player_traj.obs))):
            ...
            action = player_traj.actions[idx] if idx < len(player_traj.actions) else None
            episode.append(Step(pid=player_traj.pid, obs=player_traj.obs[idx], prompt=player_traj.prompts[idx], prompt_ids=player_traj.prompt_ids[idx], completion=player_traj.completions[idx], completion_ids=player_traj.completion_ids[idx], completion_logprobs=player_traj.completion_logprobs[idx], reward=step_reward, env_id=env_id, step_info={"raw_reward": player_traj.final_reward, "env_reward": final_reward, "step_reward": step_reward, "seed": player_traj.game_info.get("seed"), "action": action}, group_id=player_traj.group_id))
        if not episode: return
        with self.mutex:
            key = player_traj.group_id if player_traj.group_id is not None else id(episode)
            self._pending_groups.setdefault(key, []).append(episode)
            if len(self._pending_groups[key]) >= self.group_size:
                completed_group = self._pending_groups.pop(key)
                self.episodes.extend(completed_group)
                groups = self._group()
                self.logger.info(f"BUFFER episodes: {len(self.episodes)} ({len(groups)} complete groups of size {self.group_size})")
                while len(self.episodes) > self.max_buffer_size and groups:
    ...
    def stop(self):                 self.collect = False
    def size(self) -> int:          return len(self.episodes)
    def continue_collection(self):  return self.collect
    def clear(self):
        with self.mutex:
            self.episodes.clear()
