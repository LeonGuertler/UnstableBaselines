...
WANDB_RUN_NAME = f"SinglePlayer-Batch-2-Experiment-1--{MODEL_NAME.split('/')[-1]}-[{','.join([t[0] for t in TRAINING_ENVS])}]-{int(time.time())}"
...
try:
    collector.collect.remote(num_workers=150, num_eval_workers=0)
