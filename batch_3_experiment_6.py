...
WANDB_RUN_NAME = f"Batch-3-Experiment-6--{MODEL_NAME.split('/')[-1]}-[{','.join([t[0] for t in TRAINING_ENVS])}]-{int(time.time())}"


ray.init()

# initialize the tracker to keep wandb up to date and print as necessary
tracker = unstable.WandBTracker.options(name="Tracker").remote(wandb_run_name=WANDB_RUN_NAME) # , exploration_env_id=["SimpleTak-v0-train"]) for exploration metrics
...
model_pool = unstable.ModelPool.remote(
    ...
    sample_mode="random",
    keep_n_most_diverse=5, # how many diverse checkpoints to sample from the active pool
    max_active_lora=30 # how many lora checkpoints to keep active
)
