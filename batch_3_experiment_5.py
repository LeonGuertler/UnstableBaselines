...
WANDB_RUN_NAME = f"Batch-3-Experiment-5--{MODEL_NAME.split('/')[-1]}-[{','.join([t[0] for t in TRAINING_ENVS])}]-{int(time.time())}"
...
model_pool = unstable.ModelPool.remote(
    ...
    max_active_lora=10 # how many lora checkpoints to sample from
)
